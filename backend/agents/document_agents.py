...
class DocumentProcessingState:
    """State object for LangGraph document processing workflow"""
...
class ParseAgent:
    """Agent responsible for parsing documents and extracting raw content"""
...
class StructureAgent:
    """Agent responsible for structuring extracted content into hierarchical format"""
...
class ValidateAgent:
    """Agent responsible for validating extracted content"""
...
class HighlightAgent:
    """Agent responsible for creating visual highlights and grounding"""
...
class StoreAgent:
    """Agent responsible for storing final results"""
...
class DocumentProcessingWorkflow:
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        async def parse_node(state: Dict) -> Dict:
            ...
            doc_state.__dict__.update(state)
            result_state = await self.parse_agent.execute(doc_state)
            return result_state.__dict__
        
        async def structure_node(state: Dict) -> Dict:
            doc_state = DocumentProcessingState()
            doc_state.__dict__.update(state)
            result_state = await self.structure_agent.execute(doc_state)
            return result_state.__dict__
        
        async def validate_node(state: Dict) -> Dict:
            doc_state = DocumentProcessingState()
            doc_state.__dict__.update(state)
            result_state = await self.validate_agent.execute(doc_state)
            return result_state.__dict__
        
        async def highlight_node(state: Dict) -> Dict:
            doc_state = DocumentProcessingState()
            doc_state.__dict__.update(state)
            result_state = await self.highlight_agent.execute(doc_state)
            return result_state.__dict__
        
        async def store_node(state: Dict) -> Dict:
            doc_state = DocumentProcessingState()
            doc_state.__dict__.update(state)
            result_state = await self.store_agent.execute(doc_state)
            return result_state.__dict__
        ...
        workflow.add_node("highlight", highlight_node)
        workflow.add_node("store", store_node)
        
        # Add edges
        workflow.add_edge("parse", "structure")
        workflow.add_edge("structure", "validate")
        workflow.add_edge("validate", "highlight")
        workflow.add_edge("highlight", "store")
